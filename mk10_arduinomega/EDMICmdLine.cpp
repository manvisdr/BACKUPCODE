...
EdmiCMDReader::EdmiCMDReader(HardwareSerial &port, uint8_t receivePin, uint8_t transmitPin) {
  port.begin(9600, SERIAL_8N1, receivePin, transmitPin);
}
#else

EdmiCMDReader::EdmiCMDReader(HardwareSerial &port) {
  port.begin(9600);
}
#endif

EdmiCMDReader::~EdmiCMDReader() {
}

void EdmiCMDReader::init(Stream *port) {
  this->serial_ = port;
}
...
uint32_t bytearrayto32byte(uint8_t *data) {
  return ((long)data[0]) << 24 | ((long)data[1]) << 16 | ((long)data[2]) << 8 | ((long)data[3]);
}
...
bool EdmiCMDReader::check_RXReg(uint8_t *buff, uint8_t typeReg, uint8_t compare) {
  return (((uint16_t)buff[0] << 8) == ((uint16_t)typeReg << 8 | (uint16_t)compare));
}
  ...
  if (status_ == Status::Busy) {
    while (i < len) {
      ...
      if (charData[0] == CAN) {
        ...
        allData[i - 1] = ((long)charData[3]) << 24 | ((long)charData[4]) << 16 | ((long)charData[5]) << 8 | ((long)charData[6]);
        // // }
...
float EdmiCMDReader::voltR() {
  return _currentValues.voltR;
}
float EdmiCMDReader::voltS() {
  return _currentValues.voltS;
}
float EdmiCMDReader::voltT() {
  return _currentValues.voltT;
}
float EdmiCMDReader::currentR() {
  return _currentValues.currentR;
}
float EdmiCMDReader::currentS() {
  return _currentValues.currentS;
}
float EdmiCMDReader::currentT() {
  return _currentValues.currentT;
}
float EdmiCMDReader::wattR() {
  return _currentValues.wattR;
}
float EdmiCMDReader::wattS() {
  return _currentValues.wattS;
}
float EdmiCMDReader::wattT() {
  return _currentValues.wattT;
}
float EdmiCMDReader::pf() {
  return _currentValues.pf;
}
float EdmiCMDReader::frequency() {
...
float EdmiCMDReader::read_voltR() {
  ...
  if (status_ == Status::Busy) {
    ...
    if (charData[0] == CAN) {
      ...
      oneData = ((long)charData[3]) << 24 | ((long)charData[4]) << 16 | ((long)charData[5]) << 8 | ((long)charData[6]);
